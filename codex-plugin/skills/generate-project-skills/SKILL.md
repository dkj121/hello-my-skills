...
description: "Generate AI-readable project guidance skills (test, code-review, core-docs-update) tailored to this codebase, and install them into .claude/skills/ and .agents/skills/. These provide context for AI to choose appropriate workflows, not hardcoded execution chains."
---
...
Generate three AI-readable guidance skills for the current project — test, code-review, core-docs-update — tailored to this codebase. These skills capture project-specific facts (test commands, standards sources, doc locations) that AI reads when choosing appropriate workflows. They are **guidance documents for AI context**, not user-invocable workflows or hardcoded execution chains.
...
1. **Study the project**: Build a fact sheet:
   ...
   - Documents: which of ARCHITECTURE.md / SPEC.md / WORKFLOW.md exist and where (Glob for them); other docs (README, docs/, ADRs, CONTRIBUTING)
...
2. **Read the templates**: Read bundled guidance templates:
   ...
   - `templates/core-docs-update/SKILL.md`
...
4. **Generate skills via subagents**: Launch three subagents in parallel to generate each skill independently. Do not proceed until all subagents complete:
   
   For each template (test, code-review, core-docs-update):
   - Launch subagent with task: "Fill the template at `templates/{name}/SKILL.md` with project facts and write to both `.claude/skills/{name}/SKILL.md` and `.agents/skills/{name}/SKILL.md`"
   - Pass the fact sheet from step 1 as context
   - Each subagent fills placeholders, sets guidance frontmatter, and installs to both locations
   - Wait for all three subagents to complete before proceeding
   
   **If subagents not available:** Fill templates inline sequentially, but still write to both locations.

5. **Set guidance frontmatter**: Ensure each generated skill has:
   ```yaml
...
6. **Verify installations**: Confirm all three skills exist at both:
...
- Confirmation that all subagents completed successfully (if used)
- Reminder: **These are AI-readable guidance documents**. AI uses them as context when choosing mature capabilities or implementing workflows directly. They do not prescribe hardcoded workflow chains.
